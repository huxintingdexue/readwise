...
### 已完成
- ✅ 人物列表卡片主容器从独立样式切换为复用 `article-card`
- ✅ 人物卡片宽度、位置、边框、阴影与文章列表卡片保持完全一致
- ✅ 去除人物卡片主容器重复定义，避免两套样式漂移
...
### 已完成
- ✅ 今日页标签筛选顺序调整为：`全部 / 产品 / 科技 / 商业 / 人生哲学`
- ✅ 同步更新前端显示顺序与 JS 标签选项顺序，避免 UI 与逻辑不一致
...
### 已完成
- ✅ 管理员“今日概览”统计改为按 `Asia/Shanghai` 计算今日
- ✅ 管理员“本周阅读排行”改为按 `Asia/Shanghai` 周起点计算
- ✅ 覆盖指标：今日活跃用户、独立访客、今日文章打开、今日活跃用户名单、本周完读排行
...
### 已完成
- ✅ 卡片顶部发布时间改为相对时间分层显示：
- ✅ `<1小时` 刚刚、`<24小时` X小时前、`1天` 昨天、`2-6天` X天前
- ✅ `1-3周` X周前、`1-11个月` X个月前、`>=1年` 回退 `YYYY/M/D`
...
## 2026-03-28 — 卡片日期上移与底部信息隐藏（v2.16.20）

### 已完成
- ✅ 卡片底部不再展示标签（`tag` 字段保留，用于后续筛选）
- ✅ 卡片底部不再展示日期
- ✅ 顶部作者信息由“阅读时长”改为“发表日期”
...
- ✅ 批量脚本调用 DeepSeek 四选一标签（科技/商业/产品/人生哲学）并回写 `articles.tag`
...
- ✅ 历史快讯独立列表页（在今日 tab 内切换），按 published_at 倒序展示所有快讯
...
### 已完成
- ✅ 修复“退出后再进入阅读进度变小”问题
- ✅ 阅读恢复增加多次延迟校准（0ms/350ms/1200ms）
- ✅ 图片加载完成后再次校准恢复位置，用户手动滚动后停止自动校准
...
### 已完成
- ✅ Service Worker 静态缓存版本升级到 `v3`，触发一次性缓存刷新
...
## 2026-03-24 — 阅读页头部图标风格统一（v2.15.3）

### 已完成
- ✅ 阅读页返回按钮改为线性箭头风格并去掉背景框
- ✅ 阅读页分享按钮改为线性三节点风格并去掉背景框
- ✅ 阅读页设置按钮改为线性菜单（汉堡）风格并去掉背景框
- ✅ 划线按钮保持现有样式不变
...
## 2026-03-24 — 阅读页分享按钮视觉更新（v2.15.2）

### 已完成
- ✅ 阅读页顶部“分享本文”按钮图标改为三节点连线风格
- ✅ 分享按钮样式改为浅米色背景 + 青绿色图标，更贴近目标视觉
...
## 2026-03-24 — 分享页去重标题与浮窗文案调整（v2.15.1）

### 已完成
- ✅ 分享页正文渲染前新增“首行重复标题去重”：若正文第一行（含 `#` 标题）与页面标题相同，自动移除
- ✅ 浮窗主标题由“阅读更多精选内容？”调整为“阅读更多精选？”
...
### 已完成
- ✅ 分享页浮窗主标题由“继续阅读更多精选？”调整为“阅读更多精选内容？”
...
## 2026-03-24 — 分享页文案与信息结构调整（v2.14.6）

### 已完成
- ✅ 顶部品牌区改为“今日硅谷”，并新增灰色小号副标题“全球一手信息触手可及”
- ✅ 文章标题下方元信息改为仅显示“作者 + 时间”，移除来源/“人工导入”展示
- ✅ 悬浮导流条文案更新为“硅谷大佬，零距离 / 每日同步硅谷圈最新一手信息”
- ✅ Android 按钮文案由“下载安卓版”改为“立即下载”
...
### 已完成
- ✅ 修复今日活跃用户名单查询在 PostgreSQL 下的 `DISTINCT + ORDER BY` 兼容问题
- ✅ 今日活跃用户数与用户名单口径保持一致，避免“人数有值但名单为 -”
...
## 2026-03-22 — 今日概览活跃用户修复（v2.12.8）

### 已完成
- ✅ 管理员统计“今日活跃用户”改为基于今日所有事件去重用户统计（不再仅限 `open_app`）
- ✅ 过滤空 `user_id`，避免“活跃用户数有值但用户名为空”导致前端显示 `-`
...
### 已完成
- ✅ 阅读页右上角将“颜色切换”升级为统一“外观”入口（避免小屏三按钮拥挤）
- ✅ 新增外观底部面板：同屏提供主题（标准/护眼/深色）与字体（阅读宋体/清晰黑体/系统默认）
- ✅ “我的”页升级为“阅读外观”分组，新增字体设置并与阅读页轻入口状态同步
- ✅ 新增字体偏好本地持久化（`rw_font_preset`），应用到全站（列表/阅读/我的）
...
## 2026-03-21 — 文案乱码修复（v2.11.4）

### 已完成
- ✅ 修复首页文章列表状态文案乱码（已读/未读/翻译中/暂无摘要等）
- ✅ 修复管理区空态和操作文案乱码（暂无邀请码、取消隐藏、加载失败等）
- ✅ 修复主题切换与错误提示等 toast 文案乱码
...
## 2026-03-20 — 启动性能优化（v2.11.2）

### 已完成
- ✅ 阅读页重功能改为按需加载：`highlight.js`、`notes.js` 在首次进入阅读页时再初始化
- ✅ 首屏启动不再等待列表网络请求完成：先进入页面并关闭开屏，列表后台拉取
- ✅ Service Worker 注册延后 1.2 秒，减少启动阶段主线程竞争
...
### 已完成
- ✅ **列表摘要展示**：文章卡片摘要由 2 行截断调整为 3 行截断，便于预览更多内容
...
## 2026-03-19 — 划线交互与跨用户展示优化（v2.10.25）

### 已完成
- ✅ **跨段落划线**：划线 DOM 包裹逻辑改为按文本节点分段包裹，跨段落选区也可成功创建
- ✅ **句柄拖动滚动策略**：选区句柄拖动时默认锁定页面滚动，仅在接近顶部/底部一行区域时自动滚动
- ✅ **跨用户样式区分**：阅读页可拉取同文他人划线，并以底部虚线样式显示
- ✅ **列表长按反馈移除**：文章卡片禁用长按选中高亮/触摸高亮视觉反馈
- ✅ **待办更新**：补充“文章列表卡片长按交互（暂隐藏）”与“分享文章”条目
...
### 已完成
- ✅ 打开管理员控制台时写入 `admin` 历史状态
- ✅ `popstate` 优先处理管理员控制台关闭
- ✅ 手机右滑返回从管理员后台回到“我的”，不再直接退出应用
