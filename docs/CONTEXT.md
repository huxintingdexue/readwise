...
- 最近变更：Android APK WebView 壳场景同步关闭阅读自动恢复，避免回读后返回直退应用 ✅
- 最近变更：Android 桌面图标启动（PWA）场景临时关闭阅读自动恢复，优先规避右滑直退与卡死风险 ✅
- 最近变更：自动恢复阅读前强制构建 `home -> home_restore_anchor -> reader` 历史栈，修复右滑返回直退应用 ✅
