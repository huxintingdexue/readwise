...
- 最近变更：今日页新增标签筛选栏（全部/科技/商业/产品/个人成长），支持横向滑动与筛选状态持久化 ✅
- 最近变更：PostHog 历史回灌修正为顶层 `timestamp`，并使用 `source=readwise-web-backfill-v2` 标记（支持稳定过滤）✅
- 最近变更：新增 7 天 PostHog 回灌脚本（含超时/重试/退避/并发配置），已完成一轮 `991/991` 全量补发 ✅
- 最近变更：`/api/articles` 排序改为未读优先、已读置后；当天 `daily_brief` 快讯固定置顶（Asia/Shanghai）✅
- 最近变更：修复前端列表二次排序覆盖后端排序，首页快讯优先展示当天快讯（无当天回退最新）✅
...
- 最近变更：新增文章 `tag` 字段与打标能力（批量脚本 + ingest 自动打标），用于列表筛选（当前卡片不展示标签）✅
